...
const included = [
  {
    name: 'Rooftop Pool',
    desc: 'Access to the infinity-edge pool with 180° sea views — complimentary for all guests.',
    path: 'M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7z',
  },
  {
    name: 'Buffet Breakfast',
    desc: 'A generous daily spread of local and continental fare served each morning.',
    path: 'circle|ray',
  },
  {
    name: 'Free Parking',
    desc: 'Private on-site parking included at no extra charge — a rarity in central Vlorë.',
    path: 'car',
  },
  {
    name: '24h Front Desk',
    desc: 'The team is always on — late arrivals, local recommendations, room service.',
    path: 'users',
  },
]
    ...
    <>
      ...
      <section className="px-[var(--gutter)] py-[clamp(4rem,7vw,6rem)]" style={{ background: 'var(--cream)' }}>
        <div className="mx-auto mb-12" style={{ maxWidth: 'var(--max-w)' }}>
          <Reveal><SectionLabel>All Rooms</SectionLabel></Reveal>
          <Reveal delay={0.05}><h2 className="font-[family-name:var(--font-head)] italic text-[clamp(1.8rem,3.5vw,2.6rem)] mt-3" style={{ color: 'var(--bark)' }}>Choose your perspective.</h2></Reveal>
        </div>
              ...
              <div className="rounded-[var(--radius)] overflow-hidden border group transition-all duration-350 hover:-translate-y-1 hover:shadow-[0_12px_40px_rgba(28,22,16,0.12)]" style={{ background: 'var(--white)', borderColor: 'var(--border)' }}>
                ...
                <div className="p-8 pb-8">
                  ...
                  <div className="flex items-center justify-between gap-4 flex-wrap">
                    <Link href={`/rooms/${room.slug}`} className="inline-flex items-center justify-center text-[0.75rem] font-medium tracking-[0.14em] uppercase px-7 py-[0.8rem] rounded-[var(--radius)] border-[1.5px] transition-all duration-300 hover:-translate-y-px hover:bg-[var(--wood)] hover:text-[var(--white)] cursor-pointer" style={{ color: 'var(--wood)', borderColor: 'var(--wood)' }}>View Room</Link>
                    <Link href="/booking" className="inline-flex items-center justify-center text-[0.75rem] font-medium tracking-[0.14em] uppercase px-7 py-[0.8rem] rounded-[var(--radius)] transition-all duration-300 hover:-translate-y-px cursor-pointer" style={{ background: 'var(--wood)', color: 'var(--white)', border: '1.5px solid var(--wood)' }}>Reserve</Link>
                  </div>
                </div>
              </div>
            </StaggerItem>
      ...
      <section className="px-[var(--gutter)] py-[clamp(4rem,6vw,5rem)]" style={{ background: 'var(--white)' }}>
        <div className="mx-auto" style={{ maxWidth: 'var(--max-w)' }}>
          <div className="max-w-[600px] mb-12">
            <Reveal><SectionLabel>With Every Room</SectionLabel></Reveal>
            <Reveal delay={0.05}><h2 className="font-[family-name:var(--font-head)] italic text-[clamp(1.8rem,3.5vw,2.6rem)] mt-3" style={{ color: 'var(--bark)' }}>Included in every stay.</h2></Reveal>
          </div>
          <StaggerReveal className="grid md:grid-cols-4 gap-8">
            {[
              { type: 'pool', name: 'Rooftop Pool', desc: 'Access to the infinity-edge pool with 180° sea views — complimentary for all guests.' },
              { type: 'sun', name: 'Buffet Breakfast', desc: 'A generous daily spread of local and continental fare served each morning.' },
              { type: 'car', name: 'Free Parking', desc: 'Private on-site parking included at no extra charge — a rarity in central Vlorë.' },
              { type: 'users', name: '24h Front Desk', desc: 'The team is always on — late arrivals, local recommendations, room service.' },
            ].map((item) => (
              <StaggerItem key={item.name}>
                <div className="text-center">
                  <div className="mb-4"><IncludedIcon type={item.type} /></div>
      ...
      <section className="px-[var(--gutter)] py-[clamp(4rem,7vw,6rem)] text-center" style={{ background: 'var(--bark)' }}>
        <Reveal>
          <SectionLabel light center>Book Direct</SectionLabel>
          <h2 className="font-[family-name:var(--font-head)] italic text-[clamp(2rem,5vw,3.5rem)] mt-4 mb-6" style={{ color: 'var(--white)' }}>Reserve directly — save 5%.</h2>
          <p className="max-w-[420px] mx-auto mb-8 text-[0.9rem]" style={{ color: 'rgba(212,196,168,0.7)' }}>Best rate guaranteed when you book through this website. No booking fees, no intermediaries.</p>
          <Link href="/booking" className="inline-flex items-center justify-center text-[0.75rem] font-medium tracking-[0.14em] uppercase px-8 py-[0.85rem] rounded-[var(--radius)] transition-all duration-300 hover:-translate-y-px cursor-pointer" style={{ background: 'var(--wood)', color: 'var(--white)', border: '1.5px solid var(--wood)' }}>Check Availability</Link>
        </Reveal>
      </section>
    </>
  )
