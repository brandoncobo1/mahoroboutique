...
const howToGetHere = [
  {
    icon: 'car',
    title: 'By Car',
    desc: 'From Tirana, take the SH4 south through Fier to Vlorë — approximately 2 hours. From Saranda or the south coast, take the SH8 northward. Free parking is available on-site; please request a space when booking.'
  },
  {
    icon: 'bus',
    title: 'By Bus',
    desc: 'Regular bus services connect Tirana, Fier, and Berat to Vlorë. The Vlorë bus station is approximately 1.5 km from the hotel — about 5 minutes by taxi or 20 minutes on foot. We can arrange a taxi transfer from the station on request.'
  },
  {
    icon: 'plane',
    title: 'By Air',
    desc: 'The nearest international airport is Tirana International Airport (TIA), 165 km to the north. We can arrange private airport transfers — please request at least 48 hours in advance via email or WhatsApp. The drive takes approximately 2 hours depending on traffic.'
  },
]
    ...
    <>
      ...
      <section className="px-[var(--gutter)] flex flex-col justify-end pb-16" style={{ minHeight: '42vh', paddingTop: 'calc(var(--nav-h) + 4rem)', background: 'var(--bark)' }}>
        <div style={{ maxWidth: 'var(--max-w)', margin: '0 auto', width: '100%' }}>
          <Reveal><SectionLabel light>Find Us</SectionLabel></Reveal>
          <Reveal delay={0.05}>
            <h1 className="font-[family-name:var(--font-head)] italic text-[clamp(2.5rem,5vw,4rem)] leading-[1.1] mt-4" style={{ color: 'var(--white)' }}>
              Vlorë, Albanian<br /><em>Riviera.</em>
            </h1>
          </Reveal>
          <Reveal delay={0.1}>
            <p className="mt-4 text-[0.92rem] leading-[1.8] max-w-[480px]" style={{ color: 'rgba(212,196,168,0.7)' }}>
              On the Lungomare, facing the bay. The address is Rruga Egnatia — five minutes from everything in Vlorë, two hours from Tirana.
            </p>
          </Reveal>
        </div>
      </section>
      ...
      <section className="px-[var(--gutter)] py-[clamp(4rem,7vw,7rem)]" style={{ background: 'var(--cream)' }}>
        <div className="mx-auto grid md:grid-cols-2 gap-[clamp(3rem,6vw,6rem)]" style={{ maxWidth: 'var(--max-w)' }}>
          <Reveal direction="left">
            ...
            <h2 className="font-[family-name:var(--font-head)] italic text-[clamp(1.6rem,3vw,2.4rem)] mb-6" style={{ color: 'var(--bark)' }}>Mahoro Boutique Hotel</h2>
            ...
            <div className="text-[0.85rem]" style={{ color: 'var(--ink-mid)' }}>
              <div className="mb-2"><strong style={{ color: 'var(--bark)' }}>Phone:</strong> <a href="tel:+355XXXXXXXX" style={{ color: 'var(--wood)' }}>+355 XX XXX XXXX</a></div>
              <div className="mb-2"><strong style={{ color: 'var(--bark)' }}>Email:</strong> <a href="mailto:info@mahorohotel.com" style={{ color: 'var(--wood)' }}>info@mahorohotel.com</a></div>
              <div className="mb-6"><strong style={{ color: 'var(--bark)' }}>WhatsApp:</strong> <a href="https://wa.me/355XXXXXXXX" style={{ color: 'var(--wood)' }}>Message us</a></div>
            </div>
            <a href="https://maps.google.com/?q=Mahoro+Boutique+Hotel+Vlore+Albania" target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center text-[0.75rem] font-medium tracking-[0.14em] uppercase px-8 py-[0.85rem] rounded-[var(--radius)] border-[1.5px] transition-all duration-300 hover:-translate-y-px hover:bg-[var(--wood)] hover:text-[var(--white)] cursor-pointer" style={{ color: 'var(--wood)', borderColor: 'var(--wood)' }}>Open in Google Maps</a>
          </Reveal>

          <Reveal direction="right">
            <SectionLabel>Distances</SectionLabel>
            <h2 className="font-[family-name:var(--font-head)] italic text-[clamp(1.6rem,3vw,2.4rem)] mb-6" style={{ color: 'var(--bark)' }}>What's nearby</h2>
      ...
      <section className="px-[var(--gutter)] py-[clamp(4rem,7vw,6rem)]" style={{ background: 'var(--parchment)' }}>
        <div className="mx-auto" style={{ maxWidth: 'var(--max-w)' }}>
          <Reveal><SectionLabel>Getting Here</SectionLabel></Reveal>
          <Reveal delay={0.05}><h2 className="font-[family-name:var(--font-head)] italic text-[clamp(1.8rem,3.5vw,2.6rem)] mb-10" style={{ color: 'var(--bark)' }}>Three ways to arrive.</h2></Reveal>
        </div>
          ...
          {howToGetHere.map((item) => (
            <StaggerItem key={item.title}>
              <div className="p-7 rounded-[var(--radius)]" style={{ background: 'var(--cream)' }}>
                <div className="mb-4"><TransportIcon type={item.icon} /></div>
      ...
      <section className="px-[var(--gutter)] py-[clamp(4rem,7vw,6.5rem)]" style={{ background: 'var(--bark)' }}>
        <div className="mx-auto" style={{ maxWidth: 'var(--max-w)' }}>
          <Reveal><SectionLabel light>Explore</SectionLabel></Reveal>
          <Reveal delay={0.05}><h2 className="font-[family-name:var(--font-head)] italic text-[clamp(1.8rem,3.5vw,2.6rem)] mb-10" style={{ color: 'var(--white)' }}>What to see nearby.</h2></Reveal>
        </div>
