...
export default function RoomDetailContent({ room, related }: Props) {
  const { lang } = useLang()
  const tr = useT(lang).pages.roomDetail
    ...
    <>
      ...
      <section className="px-[var(--gutter)] py-[clamp(4rem,8vw,7rem)]" style={{ background: 'var(--cream)' }}>
        <div className="mx-auto grid md:grid-cols-[1fr_340px] gap-[clamp(3rem,5vw,5rem)] items-start" style={{ maxWidth: 'var(--max-w)' }}>
          <Reveal direction="left">
            <SectionLabel>{tr.theRoom}</SectionLabel>
            ...
            <div className="mt-8">
              <div className="text-[0.72rem] font-medium tracking-[0.12em] uppercase mb-4" style={{ color: 'var(--ink-light)' }}>{tr.inRoomAmenities}</div>
          ...
          <Reveal direction="right">
            <div className="rounded-[var(--radius)] p-8 sticky top-[calc(var(--nav-h)+1.5rem)]" style={{ background: 'var(--bark)' }}>
              <div className="font-[family-name:var(--font-head)] italic text-[1.3rem] mb-6" style={{ color: 'var(--white)' }}>{tr.roomDetails}</div>
      ...
      <section className="px-[var(--gutter)] py-[clamp(4rem,7vw,6.5rem)]" style={{ background: 'var(--cream)' }}>
        <div className="mx-auto" style={{ maxWidth: 'var(--max-w)' }}>
          <Reveal><SectionLabel>{tr.alsoLabel}</SectionLabel></Reveal>
          <Reveal delay={0.05}><h2 className="font-[family-name:var(--font-head)] italic text-[clamp(1.6rem,3.5vw,2.4rem)] mt-2 mb-10" style={{ color: 'var(--bark)' }}>{tr.alsoH2}</h2></Reveal>
        </div>
