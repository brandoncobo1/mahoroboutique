...
const AMENITY_ICONS = ['pool', 'sunrise', 'home', 'coffee', 'car', 'users'] as const
    ...
    <>
        ...
        <StaggerReveal className="mx-auto grid grid-cols-2 md:grid-cols-4 gap-4 text-center" style={{ maxWidth: 'var(--max-w)' }}>
          {tr.stats.map((s) => (
            <StaggerItem key={s.label}>
              <span className="font-[family-name:var(--font-head)] italic text-[clamp(2rem,4vw,2.8rem)] leading-none block" style={{ color: 'var(--bark)' }}>{s.num}</span>
              <div className="text-[0.7rem] font-medium tracking-[0.18em] uppercase mt-1" style={{ color: 'var(--ink-light)' }}>{s.label}</div>
              <div className="text-[0.72rem]" style={{ color: 'var(--ink-light)' }}>{s.desc}</div>
            </StaggerItem>
          ))}
        </StaggerReveal>
      </div>
      ...
      <section className="px-[var(--gutter)] py-[clamp(5rem,9vw,8rem)] text-center" style={{ background: 'var(--cream)' }}>
        <div className="mx-auto max-w-[680px]">
          <Reveal><SectionLabel center>{tr.manifesto.label}</SectionLabel></Reveal>
          <Reveal delay={0.05}><h2 className="font-[family-name:var(--font-head)] italic text-[clamp(2rem,4.5vw,3.2rem)] leading-[1.2] mb-7" style={{ color: 'var(--bark)' }}>{tr.manifesto.h2a}<br />{tr.manifesto.h2b}</h2></Reveal>
          <Reveal delay={0.1}><div className="mx-auto mb-7 h-px max-w-[60px]" style={{ background: 'var(--sand)' }} /></Reveal>
          <Reveal delay={0.15}><p className="text-[clamp(0.95rem,1.4vw,1.05rem)] leading-[1.9] mb-5" style={{ color: 'var(--ink-mid)' }}>{tr.manifesto.p1}</p></Reveal>
          <Reveal delay={0.2}><p className="text-[clamp(0.95rem,1.4vw,1.05rem)] leading-[1.9]" style={{ color: 'var(--ink-mid)' }}>{tr.manifesto.p2}</p></Reveal>
        </div>
      </section>
      ...
      <section className="px-[var(--gutter)] py-[clamp(4rem,7vw,6rem)]" style={{ background: 'var(--parchment)' }}>
        <div className="mx-auto" style={{ maxWidth: 'var(--max-w)' }}>
          <Reveal><SectionLabel>{tr.rooms.label}</SectionLabel></Reveal>
          <Reveal delay={0.05}><h2 className="font-[family-name:var(--font-head)] italic text-[clamp(2rem,4.5vw,3rem)] mb-8" style={{ color: 'var(--bark)' }}>{tr.rooms.h2a}<br />{tr.rooms.h2b}</h2></Reveal>
        </div>
              ...
              <Link href={r.href} className="group relative block overflow-hidden rounded-[var(--radius)] cursor-pointer" style={{ aspectRatio: '3/4' }}>
                ...
                <div className="absolute bottom-0 left-0 right-0 p-6">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-[0.65rem] tracking-[0.14em] uppercase border-b" style={{ color: 'var(--gold-light)', borderColor: 'rgba(212,196,168,0.4)' }}>{tr.rooms.discover}</span>
                  </div>
                </div>
              </Link>
            </StaggerItem>
      ...
      <section className="px-[var(--gutter)] py-[clamp(4rem,7vw,6.5rem)]" style={{ background: 'var(--bark)' }}>
        <div className="mx-auto grid md:grid-cols-[1fr_2fr] gap-[clamp(3rem,6vw,6rem)] items-start" style={{ maxWidth: 'var(--max-w)' }}>
          <Reveal direction="left">
            <SectionLabel light>{tr.amenities.label}</SectionLabel>
            <h2 className="font-[family-name:var(--font-head)] italic text-[clamp(2rem,4vw,3rem)] leading-[1.15] whitespace-pre-line" style={{ color: 'var(--white)' }}>{tr.amenities.h2}</h2>
          </Reveal>
          <StaggerReveal className="grid grid-cols-2 md:grid-cols-3 gap-7">
            {tr.amenities.items.map((item, i) => (
              <StaggerItem key={item.name}>
                <AmenityIcon type={AMENITY_ICONS[i]} />
                <div className="font-[family-name:var(--font-head)] italic text-[1.05rem] mt-3 mb-1" style={{ color: 'var(--white)' }}>{item.name}</div>
                <div className="text-[0.78rem] leading-[1.6]" style={{ color: 'rgba(212,196,168,0.65)' }}>{item.desc}</div>
              </StaggerItem>
            ))}
          </StaggerReveal>
        </div>
      ...
      <section className="px-[var(--gutter)] py-[clamp(4rem,7vw,6rem)]" style={{ background: 'var(--cream)' }}>
        <div className="mx-auto" style={{ maxWidth: 'var(--max-w)' }}>
          <Reveal><SectionLabel>{tr.reviews.label}</SectionLabel></Reveal>
          <Reveal delay={0.05}><h2 className="font-[family-name:var(--font-head)] italic text-[clamp(2rem,4vw,2.8rem)] mb-8" style={{ color: 'var(--bark)' }}>{tr.reviews.h2}</h2></Reveal>
        </div>
      ...
      <section className="px-[var(--gutter)] py-[clamp(4rem,7vw,6rem)]" style={{ background: 'var(--parchment)' }}>
        <div className="mx-auto" style={{ maxWidth: 'var(--max-w)' }}>
          <Reveal><SectionLabel>{tr.gallery.label}</SectionLabel></Reveal>
          <Reveal delay={0.05}><h2 className="font-[family-name:var(--font-head)] italic text-[clamp(2rem,4vw,2.8rem)] mb-8" style={{ color: 'var(--bark)' }}>{tr.gallery.h2}</h2></Reveal>
      ...
      <section className="px-[var(--gutter)] py-[clamp(4rem,7vw,6rem)]" style={{ background: 'var(--white)' }}>
        <div className="mx-auto" style={{ maxWidth: 'var(--max-w)' }}>
          <Reveal><SectionLabel>{tr.location.label}</SectionLabel></Reveal>
          <Reveal delay={0.05}><h2 className="font-[family-name:var(--font-head)] italic text-[clamp(2rem,4vw,2.8rem)] mb-8" style={{ color: 'var(--bark)' }}>{tr.location.h2}</h2></Reveal>
          <div className="grid md:grid-cols-2 gap-[clamp(2rem,5vw,4rem)] items-start">
            ...
            <Reveal direction="right">
              <p className="text-[0.88rem] mb-6" style={{ color: 'var(--ink-mid)' }}>{tr.location.address}</p>
      ...
      <section className="px-[var(--gutter)] py-[clamp(5rem,9vw,8rem)] text-center" style={{ background: 'var(--bark)' }}>
        <Reveal className="mx-auto max-w-[600px]">
          <SectionLabel center light>{tr.cta.label}</SectionLabel>
          <h2 className="font-[family-name:var(--font-head)] italic text-[clamp(2rem,5vw,3.8rem)] leading-[1.15] mt-4 mb-6" style={{ color: 'var(--white)' }}>{tr.cta.h2a}<br />{tr.cta.h2b}</h2>
          <p className="max-w-[480px] mx-auto mb-8 text-[0.9rem]" style={{ color: 'rgba(212,196,168,0.7)' }}>{tr.cta.p}</p>
          <Link href="/booking" className="inline-flex items-center justify-center text-[0.75rem] font-medium tracking-[0.14em] uppercase px-8 py-[0.85rem] rounded-[var(--radius)] transition-all duration-300 hover:-translate-y-px cursor-pointer" style={{ background: 'var(--wood)', color: 'var(--white)', border: '1.5px solid var(--wood)' }}>{tr.cta.btn}</Link>
        </Reveal>
      </section>
    </>
  )
